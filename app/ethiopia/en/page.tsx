...
import { placesEn as places } from "../data";
...
const highlights = [
  { href: "/ethiopia/en/discover", tag: "MARKETPLACE", title: "The city's best places", body: "Explore verified salons, barbershops and spas across Addis Ababa." },
  { href: "/ethiopia/en/how-it-works", tag: "HOW IT WORKS", title: "Four steps. One appointment.", body: "Search, choose a time and receive your confirmation." },
  { href: "/ethiopia/en/business", tag: "FOR BUSINESS", title: "Fill your chairs with clients", body: "Four flexible plans with a website made for your business." },
];
...
const cityNodes = addisAreas.map((area) => ({ ...area, name: area.nameEn }));
...
export default function EthiopiaEnglishHome() {
  const marketplacePlaces = useBackendSalons("en", places);
  const featured = useFeaturedPlaces("en", bundledFeatured);
  ...
  const todayDay = 15;
  const selectedAppointments = useMemo(() => calendarAppointments(selectedDay, month, "en"), [month, selectedDay]);
  ...
  const locatorPlaces = useMemo(() => {
    if (!mappedPlaces.length) return getAreaSalons(addisAreas[safeCityNode], "en").map((place) => ({ ...place, slug: undefined as string | undefined, tag: "" }));
    ...
    <>
      ...
      <section className="am-hero" data-am-reveal>
        ...
        <div className="am-hero-copy">
          ...
          <div className="am-hero-actions">
            <Link className="am-btn am-btn-primary" href="/ethiopia/en/register">Book a free demo<span aria-hidden="true">↗</span></Link>
            <Link className="am-btn am-btn-text" href="/ethiopia/en/discover"><i aria-hidden="true">▶</i>Explore places</Link>
          </div>
        </div>
      ...
      <section className="am-locator" data-am-reveal>
        <header className="am-locator-head">
          ...
          <Link href="/ethiopia/en/discover">Explore the marketplace <span>↗</span></Link>
        </header>
        ...
        <div className="am-locator-scroll-scene">
          ...
          <aside className="am-locator-panel">
            ...
            <Link className="am-locator-more" href="/ethiopia/en/discover">See places near {selectedCityNode.name} <span>→</span></Link>
          </aside>

            <AddisMap locations={locatorNodes} activeIndex={safeCityNode} onSelect={setActiveCityNode} language="en" />
          </div>
        </div>
        ...
        <div className="am-calendar">
          ...
          <div className="am-cal-grid" style={{ ["--offset" as string]: month * 2 % 7 } as React.CSSProperties}>
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              const appointments = calendarAppointments(day, month, "en");
      ...
      <section className="am-culture" data-am-reveal>
        ...
        <div className="am-culture-cta">
          <Link className="am-btn am-btn-primary" href="/ethiopia/en/register">Book a demo<span aria-hidden="true">↗</span></Link>
          <Link className="am-btn am-btn-text" href="/ethiopia/en/discover"><i aria-hidden="true">▶</i>Explore places</Link>
        </div>
      </section>
    </>
