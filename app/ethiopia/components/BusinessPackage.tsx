...
export default function BusinessPackage({ language: _language }: { language: "am" | "en" }) {
  void _language;
  return (
    <section className="am-package" id="package" data-am-reveal>
      ...
      <div className="am-package-grid">
        {ethiopiaPlans.map((plan) => (
          <article className={`am-package-plan${plan.featured ? " featured" : ""}`} key={plan.name}>
            ...
            <Link href="/ethiopia/en/register">Book a demo<span aria-hidden="true">→</span></Link>
      ...
      <footer className="am-package-footnote">
        ...
        <Link href="/ethiopia/en/business">Compare the business tools<span aria-hidden="true">↗</span></Link>
      </footer>
    </section>
  );
}
