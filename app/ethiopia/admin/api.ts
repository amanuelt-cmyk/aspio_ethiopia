...
export const API_BASE = process.env.NEXT_PUBLIC_ASPIO_API_URL?.replace(/\/$/, "")
...
export async function adminRequest<T>(path: string, options: RequestInit = {}, token?: string): Promise<T> {
  if (!API_BASE) throw new AdminApiError("The admin service is not ready yet. Please try again shortly.", 0, "not_configured");
  ...
  try {
    response = await fetch(`${API_BASE}/api/v1${path}`, { ...options, headers, cache: "no-store" });
  } catch {
...
export function adminUpload<T>(path: string, body: FormData, token: string, onProgress?: (progress: number) => void): Promise<T> {
  if (!API_BASE) return Promise.reject(new AdminApiError("The admin service is not ready yet. Please try again shortly.", 0, "not_configured"));
  return new Promise<T>((resolve, reject) => {
    const request = new XMLHttpRequest();
    request.open("POST", `${API_BASE}/api/v1${path}`);
