...
import { AdminApiError, adminRequest, adminUpload, login } from "../api";
    ...
    <main className="et-admin-login">
      ...
      <section className="et-login-panel">
        <form onSubmit={submit}>
          <header><span>01 / SECURE ACCESS</span><a href="/ethiopia">View website <Icon name="external" size={14} /></a></header>
...
export default function AdminApp() {
  ...
  return <main className="et-admin-shell">
    <aside className={menuOpen ? "open" : ""}>
      ...
      <footer><button className="et-admin-user" onClick={() => { setView("profile"); setMenuOpen(false); }}>{session.user.avatarUrl ? <span><img src={mediaUrl(session.user.avatarUrl)} alt="" /></span> : <span>{session.user.name.slice(0, 1).toUpperCase()}</span>}<p><b>{session.user.name}</b><small>{session.user.jobTitle || session.user.role}</small></p></button><button onClick={() => void signOut()} aria-label="Sign out"><Icon name="logout" /></button></footer>
    </aside>
    {menuOpen && <button className="et-admin-scrim" onClick={() => setMenuOpen(false)} aria-label="Close menu" />}
    <section className="et-admin-main">
      <div className="et-admin-topbar"><button onClick={() => setMenuOpen(true)} aria-label="Open menu"><Icon name="menu" /></button><div><span>{visibleNavigation.find((item) => item.id === view)?.label ?? "Overview"}</span><i />{loading ? "Updating" : "Ready"}</div><a href="/ethiopia" target="_blank" rel="noreferrer">Open website <Icon name="external" size={14} /></a></div>
