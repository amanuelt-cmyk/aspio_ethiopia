...
export default function AddisMap({ locations, activeIndex, onSelect }: AddisMapProps) {
  ...
  return (
    <div ref={rootRef} className={`am-locator-map am-open-map ${status === "ready" ? "is-live" : "is-fallback"}`}>
      <div className="am-map-toolbar">
        <span><i />{locations.length} places are mapped right now</span>
        <div>
          <button type="button" aria-label="Zoom map in" onClick={() => changeZoom(1)}>+</button>
          <button type="button" aria-label="Zoom map out" onClick={() => changeZoom(-1)}>−</button>
        </div>
      </div>

      <div className="am-ethiopia-canvas">
        <div ref={containerRef} className="am-open-map-surface" aria-label="Open map of Addis Ababa" />
        ...
        {status === "error" && (
          <div className="am-open-map-preview" aria-live="polite">
            ...
            <p>
              ...
              <span>Discover places across Addis Ababa</span>
            </p>
          </div>
        )}
        ...
        <div className="am-map-journey" aria-hidden="true">
          <span>ETHIOPIA</span><i /><span>ADDIS</span>
        </div>
      </div>

      <div className="am-map-key">
        <span><i />Selected</span>
        <span><i />Other locations</span>
      </div>
